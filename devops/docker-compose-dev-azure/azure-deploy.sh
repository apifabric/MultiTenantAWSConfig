...
resourcename="multitenantawsconfig"  # lower case, only
resourcegroup="multitenantawsconfig_rg"
...
# git clone https://github.com/apilogicserver/multitenantawsconfig.git
# cd multitenantawsconfig
...
az group create --name multitenantawsconfig_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group multitenantawsconfig_rg --sku S1 --is-linux
az container create --resource-group multitenantawsconfig_rg --name multitenantawsconfig --image apilogicserver/multitenantawsconfig:latest --dns-name-label multitenantawsconfig --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//multitenantawsconfig.westus.azurecontainer.io:5656'
